...
if (pkg.version !== "3.6.2") failures.push("package.json n'est pas en version 3.6.2");
...
for (const expected of ["TROUVER UN PARTENAIRE", "COVOITURAGE", "MES MESSAGES", "Donner mon avis", "data-filter-group=\"radius\"", "data-filter-group=\"styles\"", "data-filter-group=\"levels\"", "data-filter-group=\"roles\"", "mutual-card-simple", "getSelectedFilterValues", "filterProfilesByRadius", "submitBetaFeedback", "setTrainingInterest", "listTrainingMatches", "Connexion training", "dance_role", "DANCE_ROLES", "kizconnect-symbol.png", "subscribeToMessages", "hideConversation", "blockUser", "reportUser", "overlaps(\"styles\"", "query.in(\"level\""]) {
...
if (failures.length) {
  console.error("KizConnect V3.6.2 — échec du contrôle statique:\n- " + failures.join("\n- "));
  process.exit(1);
}

console.log("KizConnect V3.6.2 — contrôle statique OK.");
